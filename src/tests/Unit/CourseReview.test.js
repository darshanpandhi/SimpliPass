...
describe("CourseReview", () => {
  it("should render a page title and input labels for course id, name, dept, difficulty, instructor, rating and a submit button", () => {
    ...
    // Course ID
    expect(component.find("label").at(0).text()).toEqual("Course ID");

    // Name
    expect(component.find("label").at(1).text()).toEqual("Name ");

    // Dept
    expect(component.find("label").at(2).text()).toEqual("Department ");

    // Diff
    expect(component.find("label").at(3).text()).toEqual("Difficulty Level ");
    ...
    // Instructor
    expect(component.find("label").at(5).text()).toEqual("Instructor ");

    // Rating
    expect(component.find("label").at(6).text()).toEqual(" Rating ");
