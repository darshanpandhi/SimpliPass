...
describe("CourseView", () => {
  it("should render a course Header containing the course id, course name and department name and also course difficulty and its list of section ratings based on the props of coursesList and the current Course selected", () => {
    ...
    expect(component.find("div").at(2).text()).toEqual("Brad Pitt82 reviews");
    expect(component.find("div").at(3).text()).toEqual("Tom Hanks63 reviews");

    expect(component.find("div").at(4).text()).toEqual(
      "Robert Downey, Jr.95 reviews"
    );
  });
});
